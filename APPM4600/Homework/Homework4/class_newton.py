# -*- coding: utf-8 -*-
"""
Created on Fri Sep 27 14:55:24 2024
...
#p0 = 1.2
  ...
  def fpp(x):
    return (9 * np.exp(3*x) - 810 * x**4 + 
            27 * (12 * x**2 * np.exp(x) + 8 * x**3 * np.exp(x) + x**4 * np.exp(x)) - 
            9 * (2 * np.exp(2*x) + 4 * x * np.exp(2*x) + 2 * x * 2 * np.exp(2*x) + x**2 * 4 * np.exp(2*x)))
  p0 = 4

  Nmax = 100
  tol = 1e-8

  (p,pstar,info,it) = newton(fp,fpp,p0,tol, Nmax)
